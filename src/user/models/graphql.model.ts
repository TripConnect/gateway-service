...
export class AuthUser {
  ...
  // TODO: Change proto to make signIn always return non-null, invalid will be indicate via exceptions
  public static fromGrpcAuthInfo(
    authenticatedInfo: AuthenticatedInfo,
  ): AuthUser {
    return new AuthUser({
      userInfo: authenticatedInfo.getUserInfo()
        ? Self.fromGrpcUserInfo(authenticatedInfo.getUserInfo() as UserInfo)
        : undefined,
      token: authenticatedInfo.getToken()
        ? Token.fromGrpcToken(authenticatedInfo.getToken())
        : undefined,
    });
  }
}
